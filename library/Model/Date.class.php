...
abstract class Model_Date
{
     ...
     * @return string
     */
    public function getDate($timestamp, $dateRep = 'jS F Y') {
        ...
        if (date('j/n/Y', $timestamp) == date('j/n/Y', time())) {
            return 'Today, ' . date('g:ia', $timestamp);
        }

        // Is the timestamp from yesterday?
        if (date('j/n/Y', $timestamp) == date('j/n/Y', time() - 86400)) {
            return 'Yesterday';
        }
     ...
     * @param int $endDate
     */
    public function getDifference($earlyDate, $lateDate) {
